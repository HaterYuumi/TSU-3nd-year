...
public:
    ...
    friend BN power_standard(const BN& base, const BN& exponent) {
        if (exponent == BN(0)) {
            return BN(1);
        }
    ...
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // Быстрое возведение в степень (алгоритм справа налево)
    friend BN power_fast(const BN& base, const BN& exponent) {
        if (exponent == BN(0)) {
            return BN(1);
        }
        ...
        // Находим количество бит в экспоненте
        int bit_length = 0;
        BN temp = exponent;
        BN zero(0);
...
void test_power(const BN& base, const BN& exponent) {
    cout << "Base: " << base << endl;
    cout << "Exponent: " << exponent << endl;
    
    // Тест стандартного возведения в степень
    auto start = high_resolution_clock::now();
    BN result_standard = power_standard(base, exponent);
    auto end = high_resolution_clock::now();
    auto duration_standard = duration_cast<nanoseconds>(end - start);
    
    cout << "Standard power:" << endl;
    cout << "Time: " << duration_standard.count() << " ns" << endl;
    cout << "Result: " << result_standard << endl;
    cout << endl;
    
    // Тест быстрого возведения в степень (первая версия)
    start = high_resolution_clock::now();
    BN result_fast = power_fast(base, exponent);
    end = high_resolution_clock::now();
    auto duration_fast = duration_cast<nanoseconds>(end - start);
    
    cout << "Fast power:" << endl;
    cout << "Time: " << duration_fast.count() << " ns" << endl;
    cout << "Result: " << result_fast << endl;
    cout << endl;
    
    // Проверка корректности результатов
    if (result_standard == result_fast) {
        cout << "results are equal" << endl;
    } else {
        cout << "results not equal" << endl;
    }
}
